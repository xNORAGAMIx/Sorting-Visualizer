...
const Header = () => {
  ...
  const { handleBubbleSort, isSorting, handleSelectionSort, handleInsertionSort, handleMergeSort } = useContext(SortContext);
  return (
    <div className="flex justify-evenly p-4">
      <div className="flex flex-row space-x-2">
        ...
        <span className="text-4xl font-semibold text-white">rting</span>
        <span className="text-4xl font-semibold text-orange-500">Visualizer</span>
      </div>
      <ul className="flex flex-row gap-6 justify-evenly items-center">
        <li>
          <button
            onClick={handleBubbleSort}
            className={`bg-gray-900 p-3 text-white font-bold rounded-xl text-xl hover:opacity-100 shadow-lg shadow-slate-900 ${isSorting ? "opacity-50 cursor-not-allowed" : "opacity-80 cursor-pointer"}`}
            disabled={isSorting}>
            Bubble Sort
          </button>
        </li>
